...
- (BOOL)open {
    ...
    if (databaseIsOpen) {
        ...
        BOOL result = sqlite3_open([databasePath UTF8String], &sqlite3Database) == SQLITE_OK;
        database = sqlite3Database;
        databaseIsOpen = &result;
        return result;
    }
}
...
- (BOOL)close {
    if (!databaseIsOpen) {
        ...
        BOOL result = sqlite3_close(database) == SQLITE_OK;
        database = NULL;
        databaseIsOpen = &result;
        return result;
    }
    ...
    for (AUPMRepo *repo in repoArray) {
        dispatch_group_async(group, dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_HIGH, 0), ^ {
            if ([self open]) {
                ...
                HBLogError(@"Unable to open database!");
            }
        });
    }
...
- (NSArray *)cachedListOfRepositories {
    ...
    if ([self open]) {
        ...
        HBLogError(@"Unable to open databse.");
